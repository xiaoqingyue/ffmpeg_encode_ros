...
void encoder::encode(cv::Mat srcImg,int serverSockfd, const char* clientip, int clientport,
                     struct RtpPacket* rtpPacket)
{
       ...
       memcpy(picture_buf, yuvImg.data, bufLen*sizeof(unsigned char));
        ...
        if(ret < 0){
         ...
         //uint8_t* buf = (uint8_t*)malloc(5000000);
        int startCode;
        if (got_picture==1){
           ...
           if(startCode3((char*)pkt.data))
                startCode = 3;
           else
                startCode = 4;

          pkt.size -= startCode;
          rtpSendH264Frame(serverSockfd, clientip, clientport,
                            rtpPacket, (uint8_t *)(pkt.data+startCode), pkt.size);//rtp打包发送
